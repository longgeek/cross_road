...
def host_exec(api_ip, api_port, db_id='', commands='', username=''):
    """
    in the docker host to exec commands
...
def host_fdcheck(api_ip, api_port, db_id='', fds='', username=''):
    """
    in the docker host to check files and dirs.
    ...
    url = "http://" + api_ip + ":" + str(api_port) + "/" + API_VERSION + \
        "/containers/" + str(db_id) + "/host/fdcheck"
    headers = {'content-type': 'application/json'}
    data = {'fds': fds, 'username': username}
    try:
        req = requests.post(
            url=url,
            headers=headers,
